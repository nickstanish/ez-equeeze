...
public class PurchaseManager extends JPanel {
  // TODO: add more accuracy to buying items (maybe * 100 and then +/- before converting back to
  // decimal to avoid rounding 0.06999999999999651 )
  ...
  public JLabel smallSizeLabel, smallPriceLabel, mediumSizeLabel, mediumPriceLabel, largeSizeLabel,
      largePriceLabel;
  ...
  /**
   * initializes all the components of the class split into several methods for clarity
   */
  ...
  /**
   * event handler to buy items checks to make sure user has enough money and then adds the selected
   * amount to their state this can most likely be cleaned up by branching classes and data as such
  ...
  public void purchase(PurchaseSize size) {
    ...
    if (state != null) {
      if (state.money >= cost) {
        ...
        switch (lastSelected) {
          case Cups:
            if (itemAtMax(state.cups)) {
              ...
              state.cups = checkAddItem(state.cups, amount, lastSelected);
            break;
          case Ice:
            if (itemAtMax(state.ice)) {
              ...
              state.ice = checkAddItem(state.ice, amount, lastSelected);
            break;
          case Lemons:
            if (itemAtMax(state.lemons)) {
              ...
              state.lemons = checkAddItem(state.lemons, amount, lastSelected);
            break;
          case Sugar:
            if (itemAtMax(state.sugar)) {
              ...
              state.sugar = checkAddItem(state.sugar, amount, lastSelected);
            break;
        }
        ...
        if (Constants.debugging)
          System.err.println("TODO: tell user that they can't afford it " + neme);
      }
      if (Constants.debugging)
        state.print();
    }
  ...
  /**
   * makes sure that added amount when purchasing doesnt go over the max int value and cycle
   * negative
   * 
   * @param amount
   * @param add
  ...
  public static void main(String[] args) {
    ...
    frame.getContentPane().add(new PurchaseManager(null, new State(true)));
    frame.pack();
