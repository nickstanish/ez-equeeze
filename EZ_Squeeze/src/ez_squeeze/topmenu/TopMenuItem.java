...
public class TopMenuItem {
  ...
  public void printRect(Rectangle2D rect) {
    System.out.println(rect.getX() + " " + rect.getY() + " " + rect.getWidth() + " "
        + rect.getHeight());
  }
  ...
  public void draw(Graphics2D g) {
    ...
    paddedArea =
        new Rectangle2D.Double(base.x - xpadding / 2, (base.y + (index) * (height + 1))
            + fm.getDescent(), rect.getWidth() + xpadding, height + fm.getDescent());
    area =
        new Rectangle2D.Double(base.x, (base.y + (index) * (height + 1)), rect.getWidth(),
            rect.getHeight());
    if (hovered) {
